...
def get_stims():
    ...
    params["freqs"] = map(float,params["freqs"].split(","))
...
def load_from_db(params):
    trialset = []
    # assert max(params["stims"]) < len(all_trials)+1

    conds = params["conds"]

    for freqs in params["freqs"]:
        for delay in params["delays"]:
            for stim in params["stims"]:
                ...
                trialset += add_cond(trials[trial_idx].tolist(),["delay","freq"],[delay,freqs])
